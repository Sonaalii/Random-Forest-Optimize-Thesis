...
url = 'https://drive.google.com/file/d/1QAE6uk4Uv8HsufrLObetE_tu-zXGR3M4/view?usp=drive_link'
output = 'churn_model_with_threshold_.pkl'
gdown.download(url, output, quiet=False)

with open('churn_model_with_threshold_.pkl', 'rb') as f:
    model = pickle.load(f)
...
years_in_job = st.selectbox("Years in Current Job", ['< 1 year', '1 year', '2 years', '3 years', '4 years', '5 years',
                                                     '6 years', '7 years', '8 years', '9 years', '10+ years'])
home_ownership = st.selectbox("Home Ownership", ['Rent', 'Own Home', 'HaveMortgage', 'Home Mortgage'])
purpose = st.selectbox("Purpose", ['Business Loan', 'Buy House', 'Buy a Car', 'Debt Consolidation',
...
if st.button("Predict"):
    ...
    job_encoded = [1 if years_in_job == cat else 0 for cat in ['1 year', '10+ years', '2 years', '3 years', '4 years',
                                                               '5 years', '6 years', '7 years', '8 years', '9 years',
                                                               '< 1 year']]
    home_encoded = [1 if home_ownership == cat else 0 for cat in ['HaveMortgage', 'Home Mortgage', 'Own Home', 'Rent']]
    purpose_encoded = [1 if purpose == cat else 0 for cat in ['Business Loan', 'Buy House', 'Buy a Car',
                                                             'Debt Consolidation', 'Educational Expenses',
                                                             'Home Improvements', 'Medical Bills', 'Other',
                                                             'Take a Trip', 'major_purchase', 'moving', 'other',
                                                             'renewable_energy', 'small_business', 'vacation',
                                                             'wedding']]

    input_data = np.array([
        current_loan_amount, credit_score, annual_income, monthly_debt, years_credit_history,
        open_accounts, credit_problems, current_credit_balance, max_open_credit, bankruptcies,
        tax_liens, credit_score_missing
